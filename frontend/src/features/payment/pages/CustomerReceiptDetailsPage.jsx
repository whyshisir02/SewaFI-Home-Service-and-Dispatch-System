...
import { useCustomerReceipt, useDownloadCustomerReceipt } from '../hooks/useReceipts';

function CustomerReceiptDetailsPage() {
  const { receiptId, bookingId } = useParams();
  const receiptQuery = useCustomerReceipt({ receiptId, bookingId });
      ...
      <Container className="py-8">
        <Card className="rounded-2xl border border-[var(--sf-border)] bg-[var(--sf-surface)] p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Button as={Link} to={ROUTES.customer.receipts} variant="outline" className="rounded-xl">
              Back to Receipts
            </Button>
          </div>
        </Card>
  ...
  return (
    <Container className="space-y-6 py-6 lg:py-8">
      <PageHeader
        ...
        description="Keep this receipt for your completed service and support reference."
        actions={(
      ...
      <ReceiptSummaryCard receipt={receipt} showParties variant="customer" />
    </Container>
  );
}
