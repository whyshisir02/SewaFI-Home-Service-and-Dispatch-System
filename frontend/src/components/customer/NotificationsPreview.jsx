...
const getNotificationPath = (notification) =>
  notification?.bookingId ? `/customer/bookings/${notification.bookingId}` : null;

export function NotificationsPreview({ notifications = [], isLoading, isError }) {
  if (isLoading) {
    return (
  ...
  return (
    <section className="rounded-[28px] border border-[var(--sf-border)] bg-[var(--sf-surface)] p-5 shadow-[0_16px_40px_rgba(7,59,115,0.08)]">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--sf-secondary)]">Updates</p>
      ...
      {isError ? (
        <div className="mt-5 rounded-2xl border border-[var(--sf-border)] bg-[var(--sf-surface-soft)] p-4">
          <p className="font-semibold text-[var(--sf-text-main)]">Unable to load notifications right now.</p>
      ...
      {!isError && !notifications.length ? (
        <div className="mt-5">
          <EmptyState title="No notifications yet." description="Booking, provider, and payment updates will appear here." />
        </div>
      ) : null}
      ...
      {!isError && notifications.length ? (
        <div className="mt-5 space-y-3">
          {notifications.slice(0, 5).map((notification) => {
            ...
            const IconComponent = notification.isRead ? CheckCircle2 : Clock3;
            const content = (
      ...
      {/* TODO: Add /customer/notifications route when the customer notification center page is available. */}
    </section>
  );
}
