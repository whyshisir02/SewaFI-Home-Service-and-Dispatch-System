...
export function AdminDashboardHeader({ user, onRefresh }) {
  return (
    <header className="overflow-hidden rounded-[28px] border border-[var(--sf-border)] bg-[radial-gradient(circle_at_top_right,var(--sf-primary-soft),transparent_34%),linear-gradient(135deg,var(--sf-surface)_0%,var(--sf-surface-soft)_100%)] p-6 shadow-[var(--sf-shadow)] sm:p-8">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-2 text-sm font-semibold text-[var(--sf-text-soft)]">Signed in as {getUserName(user)}</p>
        </div>
      ...
      {/* TODO: Add a date-range selector when admin analytics endpoints support date-range filters. */}
    </header>
  );
}
