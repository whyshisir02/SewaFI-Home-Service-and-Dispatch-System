  ...
  return (
    <section className="rounded-3xl border border-[var(--sf-border)] bg-[var(--sf-surface)] p-4 shadow-[0_12px_30px_rgba(7,59,115,0.08)]">
      <div>
        ...
        <h2 className="mt-1 text-xl font-extrabold text-[var(--sf-text-main)] sm:text-2xl">Service Category Distribution</h2>
        <p className="mt-1 text-sm text-[var(--sf-text-muted)]">Completed bookings grouped by service category.</p>
      </div>
      ...
      {!isError && chartData.length ? (
        <div className="mt-4 grid gap-3 lg:grid-cols-[210px_1fr] lg:items-center">
          ...
          <div className="space-y-2.5">
            {chartData.slice(0, 6).map((item, index) => (
              <div key={item.id} className="flex items-center justify-between gap-3 rounded-2xl border border-[var(--sf-border)] bg-[var(--sf-surface-soft)] px-3 py-2.5">
