...
import { useCustomerReceipts, useDownloadCustomerReceipt } from '../hooks/useReceipts';
...
function CustomerReceiptsPage() {
  ...
  const receiptsQuery = useCustomerReceipts({ search, page, limit: 12 });
  const downloadMutation = useDownloadCustomerReceipt();
  ...
  return (
    <Container className="space-y-6 py-6 lg:py-8">
      <PageHeader
        ...
        description="View completed payment receipts and download them as PDF."
        actions={(
          <Button as={Link} to={ROUTES.customer.payments} variant="outline" className="h-11 rounded-xl">
            Back to Payments
          </Button>
        )}
      />
      ...
      <Card className="rounded-2xl border border-[var(--sf-border)] bg-[var(--sf-surface)] p-4">
        <Input
          ...
          placeholder="Booking code or service name"
        />
      </Card>
      ...
      {!receiptsQuery.isLoading && !receiptsQuery.isError && !receipts.length ? (
        <EmptyState
          ...
          description="Receipts appear after a booking is completed and payment is confirmed."
        />
      ) : null}
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {receipts.map((receipt) => (
            <div key={receipt.id} className="space-y-3">
              <ReceiptSummaryCard receipt={receipt} variant="customer" />
              <div className="grid gap-2 sm:grid-cols-2">
                <Button
                  as={Link}
                  to={ROUTES.customer.receiptDetails.replace(':receiptId', receipt.id)}
                  variant="outline"
